      ...
      async function getGame(parameter = "shooter") {
        try {
          ...
          let response = await fetch(
            `https://free-to-play-games-database.p.rapidapi.com/api/games?category=${parameter}`,
            options
          );
      ...
      <div onclick="closeSection()" class="col">
            <div onclick="detailsGame(${
              apiData[i].id
            }) " class="card bg-dark text-white main-card" style="width: 18rem;">
        <img src="${apiData[i].thumbnail}" class="card-img-top w-100" alt="...">
        <div class="card-body">
        <div class="d-flex justify-content-between">
      <a href="#" class="fw-bolder game-title ">${apiData[i].title}</a>
      <a href=" ${
            apiData[i].freetogame_profile_url
          }" class=" btn btn-info btn-card "> free</a>
        </div>
          <p class="card-text text-center pt-1"> ${apiData[i].short_description.slice(
            " ",
            40
          )} </p>
        </div>
